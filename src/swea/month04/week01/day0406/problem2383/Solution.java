package swea.month04.week01.day0406.problem2383;
...
public class Solution {
	...
	static int simulate(List<Person> list, Stair stair) {
		...
		// waitQ에 list에 들어있는 |PR-SR| + |PC-SC| 값을 계산 후 저장
		for (Person p : list)
	...
	static void subs(int index, boolean[] checked) {
		if (index == checked.length) {
			...
			for (int i = 0; i < checked.length; i++) {
				if (checked[i])
					list1.add(persons.get(i));
				else
					list2.add(persons.get(i));
			}
